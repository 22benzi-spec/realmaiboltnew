  ...
  <div class="page-content">
    ...
    <div class="stats-row">
      ...
      <div class="stat-card stat-loss">
        <div class="stat-val">{{ stats.loss }}</div>
        <div class="stat-label">已确认损失</div>
      </div>
    </div>

    <div class="card-panel">
      <div class="toolbar">
        <a-select v-model:value="filterStatus" style="width:130px" @change="load" allow-clear placeholder="全部状态">
        ...
        <a-select v-model:value="filterStolen" style="width:120px" @change="load" allow-clear placeholder="本金状态">
          <a-select-option :value="true">本金被骗</a-select-option>
          <a-select-option :value="false">本金未骗</a-select-option>
        </a-select>
        <a-input-search v-model:value="filterSearch" placeholder="搜索订单号/买手/ASIN..." style="width:240px" @search="load" allow-clear />
      ...
      >
        <template #bodyCell="{ column, record }">
          <template v-if="column.key === 'issue_number'">
            <a class="link-text" @click="openDetail(record)">{{ record.issue_number }}</a>
          </template>
          ...
          <template v-if="column.key === 'replacement'">
            <template v-if="record.new_amazon_order_id">
              <div class="cell-new-order">新: {{ record.new_amazon_order_id }}</div>
            </template>
            <template v-if="record.replacement_buyer_name">
              <div class="cell-replacement">补单买手: {{ record.replacement_buyer_name }}</div>
              <div v-if="record.replacement_sub_order_number" class="cell-meta">{{ record.replacement_sub_order_number }}</div>
            </template>
            <span v-if="!record.new_amazon_order_id && !record.replacement_buyer_name" class="text-gray">--</span>
          </template>
    ...
    <a-drawer
      ...
      :title="drawerMode === 'create' ? '新建问题单' : `问题单 ${currentIssue?.issue_number || ''}`"
      width="680"
      placement="right"
      destroy-on-close
    >
      <template v-if="drawerMode === 'create'">
        <a-form layout="vertical">
          <a-form-item label="选择问题子订单" required>
            <a-select
              v-model:value="createForm.sub_order_id"
              show-search
              :filter-option="false"
              placeholder="输入子订单号搜索..."
              @search="searchSubOrders"
              @change="onSubOrderSelect"
              style="width:100%"
              :not-found-content="searchingSubOrders ? '搜索中...' : '无结果'"
            >
              <a-select-option v-for="so in subOrderResults" :key="so.id" :value="so.id">
                <div class="so-option">
                  <span class="so-option-no">{{ so.sub_order_number }}</span>
                  <span class="so-option-meta">{{ so.asin }} · {{ so.store_name }}</span>
                </div>
              </a-select-option>
            </a-select>
          </a-form-item>

          <div v-if="selectedSubOrder" class="selected-sub-order-card">
            <div class="so-card-row"><span class="so-card-label">子订单号</span><span class="so-card-val mono">{{ selectedSubOrder.sub_order_number }}</span></div>
            <div class="so-card-row"><span class="so-card-label">ASIN</span><span class="so-card-val">{{ selectedSubOrder.asin }}</span></div>
            <div class="so-card-row"><span class="so-card-label">店铺</span><span class="so-card-val">{{ selectedSubOrder.store_name }}</span></div>
            <div class="so-card-row"><span class="so-card-label">产品价格</span><span class="so-card-val amount-usd">${{ Number(selectedSubOrder.product_price || 0).toFixed(2) }}</span></div>
            <div class="so-card-row"><span class="so-card-label">买手</span><span class="so-card-val">{{ selectedSubOrder.buyer_name || '未分配' }}</span></div>
            <div class="so-card-row"><span class="so-card-label">业务员</span><span class="so-card-val">{{ selectedSubOrder.staff_name || '--' }}</span></div>
            <div class="so-card-row"><span class="so-card-label">亚马逊订单号</span><span class="so-card-val mono">{{ selectedSubOrder.amazon_order_id || '--' }}</span></div>
          </div>

          <a-form-item label="问题类型" required>
            <a-radio-group v-model:value="createForm.issue_type">
              <a-radio-button v-for="t in issueTypes" :key="t" :value="t">{{ t }}</a-radio-button>
            </a-radio-group>
          </a-form-item>

          <a-form-item label="问题描述">
            <a-textarea v-model:value="createForm.description" :rows="3" placeholder="描述具体问题..." />
          </a-form-item>
        </a-form>
      ...
      <template v-else-if="currentIssue">
        <div class="drawer-section">
          ...
          <div class="detail-grid">
            <div class="detail-item">
              <span class="detail-label">问题单号</span>
              <span class="detail-val mono">{{ currentIssue.issue_number }}</span>
            </div>
            <div class="detail-item">
            ...
            <div class="detail-item">
              ...
              <a-tag :color="statusColorMap[currentIssue.issue_status] || 'default'">{{ currentIssue.issue_status }}</a-tag>
            </div>
            <div class="detail-item">
              <span class="detail-label">旧亚马逊订单号</span>
              <span class="detail-val mono">{{ currentIssue.old_amazon_order_id || '--' }}</span>
            </div>
            <div v-if="currentIssue.description" class="detail-item full">
        ...
        <div class="drawer-section">
          <div class="drawer-section-title">本金状态</div>
          <div class="principal-section">
            <a-radio-group v-model:value="editForm.principal_stolen" @change="onPrincipalChange">
        ...
        <div v-if="!editForm.principal_stolen" class="drawer-section">
          <div class="drawer-section-title">替换订单号 (老外提供新订单号)</div>
          <a-form layout="vertical">
            <a-form-item label="新亚马逊订单号">
              <a-input v-model:value="editForm.new_amazon_order_id" placeholder="输入老外提供的新订单号..." />
            </a-form-item>
            <a-form-item label="差价盈利 (退款少退的部分)">
        ...
        <div v-if="editForm.principal_stolen" class="drawer-section">
          <div class="drawer-section-title">补单处理 (需找新买手)</div>
          <a-form layout="vertical">
        ...
        <div class="drawer-section">
          ...
          <a-textarea v-model:value="editForm.resolution_notes" :rows="3" placeholder="记录处理过程..." />
        </div>

        <div class="drawer-section">
          <div class="drawer-section-title">更新状态</div>
          <a-radio-group v-model:value="editForm.issue_status">
...
const filterStolen = ref<boolean | undefined>(undefined)
const filterSearch = ref('')
const drawerOpen = ref(false)
const drawerMode = ref<'create' | 'detail'>('create')
const currentIssue = ref<any>(null)
const pagination = ref({ current: 1, pageSize: 20, total: 0 })

const issueTypes = ['不下单', '取消', '退款', '无此订单']
const statusOptions = ['待处理', '处理中', '已替换订单', '需补单', '已补单', '已确认损失', '已关闭']
...
const stats = ref({ pending: 0, processing: 0, replaced: 0, needReorder: 0, reordered: 0, loss: 0 })

const subOrderResults = ref<any[]>([])
const searchingSubOrders = ref(false)
const selectedSubOrder = ref<any>(null)
const buyerResults = ref<any[]>([])
const searchingBuyers = ref(false)

const createForm = reactive({
  sub_order_id: undefined as string | undefined,
  issue_type: '不下单',
  description: '',
})
...
const columns = [
  { title: '问题单号', key: 'issue_number', width: 160 },
  { title: '子订单信息', key: 'sub_info', width: 180 },
  { title: '买手/业务员', key: 'buyer', width: 120 },
  { title: '问题类型', key: 'issue_type', width: 90 },
  { title: '本金', key: 'principal', width: 100 },
  { title: '状态', key: 'issue_status', width: 100 },
  { title: '替换/补单', key: 'replacement', width: 180 },
  {
    title: '创建时间',
...
async function loadStats() {
  ...
  stats.value = {
    ...
    loss: data.filter(d => d.issue_status === '已确认损失').length,
  }
}
...
async function load() {
  ...
  try {
    ...
    if (filterStolen.value !== undefined && filterStolen.value !== null) query = query.eq('principal_stolen', filterStolen.value)
    if (filterSearch.value) {
      const kw = `%${filterSearch.value}%`
      query = query.or(`sub_order_number.ilike.${kw},buyer_name.ilike.${kw},asin.ilike.${kw},issue_number.ilike.${kw},old_amazon_order_id.ilike.${kw}`)
    }
...
let subOrderSearchTimer: ReturnType<typeof setTimeout> | null = null
function searchSubOrders(keyword: string) {
  if (subOrderSearchTimer) clearTimeout(subOrderSearchTimer)
  if (!keyword || keyword.length < 2) { subOrderResults.value = []; return }
  subOrderSearchTimer = setTimeout(async () => {
    searchingSubOrders.value = true
    const kw = `%${keyword}%`
    const { data } = await supabase
      .from('sub_orders')
      .select('id, sub_order_number, asin, store_name, product_price, buyer_id, buyer_name, staff_id, staff_name, amazon_order_id, order_id, customer_name')
      .or(`sub_order_number.ilike.${kw},asin.ilike.${kw},amazon_order_id.ilike.${kw}`)
      .limit(20)
    subOrderResults.value = data || []
    searchingSubOrders.value = false
  }, 300)
}

function onSubOrderSelect(subOrderId: string) {
  selectedSubOrder.value = subOrderResults.value.find(s => s.id === subOrderId) || null
}
...
function searchBuyers(keyword: string) {
  ...
  if (!keyword || keyword.length < 1) { buyerResults.value = []; return }
...
function onPrincipalChange() {
  if (!editForm.principal_stolen) {
    ...
    editForm.replacement_buyer_id = undefined
    editForm.replacement_buyer_name = ''
    editForm.replacement_sub_order_number = ''
    editForm.blacklist_buyer = false
  } else {
    editForm.new_amazon_order_id = ''
    editForm.profit_diff = 0
    editForm.principal_amount = Number(currentIssue.value?.product_price || 0)
    editForm.loss_amount = Number(currentIssue.value?.product_price || 0)
  }
}

function openCreate() {
  drawerMode.value = 'create'
  Object.assign(createForm, { sub_order_id: undefined, issue_type: '不下单', description: '' })
  selectedSubOrder.value = null
  subOrderResults.value = []
...
async function handleCreate() {
  if (!createForm.sub_order_id) {
    message.warning('请选择子订单')
    return
  }
  saving.value = true
  try {
    const so = selectedSubOrder.value
    const orderNumber = await getOrderNumber(so.order_id)
...
async function getOrderNumber(orderId: string | null): Promise<string> {
  if (!orderId) return ''
  const { data } = await supabase.from('erp_orders').select('order_number').eq('id', orderId).maybeSingle()
...
async function handleUpdate() {
  ...
  try {
    ...
    if (['已替换订单', '已补单', '已确认损失', '已关闭'].includes(editForm.issue_status)) {
    ...
    if (editForm.blacklist_buyer && currentIssue.value.buyer_id) {
      await supabase.from('erp_buyers').update({
        status: '黑名单',
        blacklist_reason: `售后问题单 ${currentIssue.value.issue_number} - 本金被骗`,
...
.stat-loss .stat-val { color: #6b7280; }
...
.cell-replacement { font-size: 11px; color: #374151; }
...
.principal-section { display: flex; align-items: center; flex-wrap: wrap; gap: 8px; }
.blacklist-action { margin-top: 12px; padding: 10px 14px; background: #fef2f2; border: 1px solid #fecaca; border-radius: 8px; }

.so-option { display: flex; flex-direction: column; }
.so-option-no { font-family: 'Courier New', monospace; font-size: 12px; font-weight: 600; }
.so-option-meta { font-size: 11px; color: #9ca3af; }
...
.so-card-row { display: flex; align-items: center; gap: 8px; }
.so-card-label { font-size: 11px; color: #6b7280; min-width: 80px; }
.so-card-val { font-size: 13px; color: #1a1a2e; }
.so-card-val.mono { font-family: 'Courier New', monospace; font-size: 12px; }
